...
from app.schemas.case import CaseQueryRequest,AnalysisResultRequest,PGLoopSearch
...
# 异步日志记录
loguru_logger.add("case_temp.log", enqueue=True)  

router_ex_temp = APIRouter() 
...
async def background_case_analysis_request_post(request_id, caller_identity_id, case_number,transcripts):
    ...
    for transcript in transcripts:
        ...
        records_to_create.append(CaseAnalysisRequestRecord(
            ...
            transcript_number=transcript.get('zjhm'),
            ...
            transcript_time=transcript.get('kssj').strftime("%Y-%m-%d %H:%M:%S"), # 增加一个笔录记录时间
    ...
    if payloads_to_send:
        ...
        tasks = [
            _send_with_semaphore(settings.LLM_URL_TEMP, payload, semaphore)
            for payload in payloads_to_send
        ]
        await asyncio.gather(*tasks)

# 外部通过按键编号查询笔录的接口
@router_ex_temp.post("/caseAnalysis", summary="按案件编号查询笔录", 
    dependencies=[
...
async def case_analysis(
    ...
    try:
        async with get_db_connection() as conn:
            results = await conn.fetch(settings.QUERY_GET_BY_TEMP, data.page_size, data.offset)
    ...
    if transcript_count == 0:
        ...
        raise HTTPException(status_code=501, detail=f"未查询到相关笔录记录")
    else:
        await CaseAnalysisRequest.create(
            request_id=request_id,
            case_number=case_number,
...
async def background_case_analysis_result_recieve(data: AnalysisResultRequest):
    try:
        ...
        for re in records_list:
            ask_type.append(re["ask_type"])
            interviewee_name.append(re["interviewee_name"])
            try:
        ...
        loguru_logger.info(analysis_results)
...
@router_ex_temp.post("/caseAnalysisResult", summary="接收笔录分析结果")
